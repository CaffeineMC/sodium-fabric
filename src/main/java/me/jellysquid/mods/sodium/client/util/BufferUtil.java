...
public class BufferUtil {
     ...
     * @param buffer The destination buffer
     */
    public static void copyIntArraySlice(int[] data, int start, int limit, int offset, ByteBuffer buffer) {
        if (limit + start > data.length) {
            throw new IllegalArgumentException("Source array is too small");
        }
        ...
        if (UnsafeUtil.isAvailable() && buffer.isDirect()) {
            copyIntArrayUnsafe(data, start, limit, offset, MemoryUtil.memAddress(buffer));
        } else {
            copyIntArrayDefault(data, start, limit, offset, buffer);
        }
    }

    private static void copyIntArrayUnsafe(int[] data, int start, int limit, int offset, long addr) {
        UnsafeUtil.instance().copyMemory(data, UnsafeUtil.INT_ARRAY_OFFSET + start, null, addr + offset, limit * 4);
    }

    private static void copyIntArrayDefault(int[] data, int start, int limit, int offset, ByteBuffer buffer) {
        for (int i = 0; i < limit; i++) {
            buffer.putInt(offset + (i * 4), data[start + i]);
        }
    }
