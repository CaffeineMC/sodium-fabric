...
public class OptionImpl<S, T> implements Option<T> {
    ...
    private OptionImpl(OptionStorage<S> storage, Text name, Text tooltip, OptionBinding<S, T> binding, Function<OptionImpl<S, T>, Control<T>> control, EnumSet<OptionFlag> flags, OptionImpact impact, boolean enabled) {
