...
public class DriverProbeLauncher {
    ...
    public static DriverProbeResult launchProbe() {
        var lwjglCoreJar = Validate.notNull(LibraryClasspaths.getClasspathEntry(Library.class), "Could not find LWJGL Core library path");
        var lwjglOpenGlJar = Validate.notNull(LibraryClasspaths.getClasspathEntry(GL30C.class), "Could not find LWJGL OpenGL library path");
        var lwjglGlfwJar = Validate.notNull(LibraryClasspaths.getClasspathEntry(GLFW.class), "Could not find LWJGL GLFW library path");
        var modJarPaths = Validate.notNull(ModClasspaths.getClasspathEntriesForMod(DriverProbeEntrypoint.class, "sodium"), "Could not find driver prober library path");
        var lwjglCoreNativeName = Validate.notNull(Configuration.LIBRARY_NAME.get(Platform.mapLibraryNameBundled("lwjgl")), "Couldn't determine LWJGL Core native name");
        var lwjglCoreNative = Validate.notNull(LwjglNatives.findSystemNative("org.lwjgl", lwjglCoreNativeName), "Couldn't find LWJGL Core native path");
        var lwjglOpenGlNativeName = Validate.notNull(Platform.mapLibraryNameBundled("lwjgl_opengl"), "Couldn't determine LWJGL OpenGL native name");
        var lwjglOpenGlNative = Validate.notNull(LwjglNatives.findSystemNative("org.lwjgl.opengl", lwjglOpenGlNativeName), "Couldn't find LWJGL OpenGL native path");
        var lwjglGlfwNativeName = Validate.notNull(Configuration.GLFW_LIBRARY_NAME.get(Platform.mapLibraryNameBundled("glfw")), "Couldn't determine LWJGL GLFW native name");
        var lwjglGlfwNative = Validate.notNull(LwjglNatives.findBundledNative("org.lwjgl.glfw", lwjglGlfwNativeName), "Couldn't find LWJGL GLFW native path");
        var jvmExecutable = Validate.notNull(findJvmExecutable(), "Couldn't find currently running JVM executable");
