...
//
//    private RenderList<MdiCountChunkRenderBatch>[] renderLists;
//
...
//        RenderList<MdiCountChunkRenderBatch>[] renderLists = new RenderList[totalPasses];
//
...
//        // Alignment to pointers means that memory copies may gain a slight speed boost.
//        int stackSize = (MathUtil.align(instanceBufferPassSize, Pointer.POINTER_SIZE) +
//                         MathUtil.align(commandBufferPassSize, Pointer.POINTER_SIZE)) * totalPasses;
//        this.ensureStackCapacity(stackSize);
//
//        try (MemoryStack stack = this.tempBufferStack.push()) {
//            for (ChunkRenderPass renderPass : chunkRenderPasses) {
//                renderLists[renderPass.getId()] = new RenderList<>(
//                        stack.nmalloc(commandBufferPassSize),
//                        stack.nmalloc(instanceBufferPassSize)
//                );
//            }
//
//            boolean reverseOrder = false; // TODO: fix me
//
//            for (Iterator<SortedChunkLists.RegionBucket> bucketIterator = chunks.sortedRegionBuckets(reverseOrder); bucketIterator.hasNext(); ) {
//                SortedChunkLists.RegionBucket regionBucket = bucketIterator.next();
//
...
//                        }
//
//                        RenderList<MdiCountChunkRenderBatch> renderList = renderLists[model.pass.getId()];
//
//                        if (renderList == null) { // very bad
//                            continue;
//                        }
//
//                        for (long range : model.modelParts) {
//                            if ((visibility & range) == 0) {
//                                continue;
//                            }
//
//                            int vertexCount = UploadedChunkGeometry.PackedModel.unpackPartVertexCount(range);
//                            int firstVertex = baseVertex + UploadedChunkGeometry.PackedModel.unpackPartFirstVertex(range);
//
...
//                }
//
//                for (RenderList<MdiCountChunkRenderBatch> renderList : renderLists) {
...
//                RenderList<MdiCountChunkRenderBatch> renderList = renderLists[i];
//
